...
import type { NotificationColumnProps } from "@/pages/create-template/types/notification.type";
...
const NotificationColumn: React.FC<NotificationColumnProps> = ({
  ...
  return (
    <Box className={clsx({ [className]: true })}>
      <Box className="notifications-card__title">
        {title}
        {iconVisible && (
          <CSvgIcon
            component={icon}
      ...
      <Box
        className={clsx({
          "notifications-card__content": true,
        })}
      >
        {truncatedValue}
