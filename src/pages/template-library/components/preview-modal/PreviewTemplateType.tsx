...
import { RenderDropdownQues, RenderRadioQues, RenderUserInputQues } from "./PreviewQuesType";
...
export const renderAnswer = (question, isDesktopPreview, templateType) => {
    const { DROPDOWN, CHECKBOX, RADIO_BUTTON, LONG_USER_INPUT, USER_INPUT } = QUESTION_TYPES;
    const type = question?.questionType;
    switch (type) {
        case DROPDOWN.value:
        case CHECKBOX.value:
            return <RenderDropdownQues question={question} isDesktopPreview={isDesktopPreview} templateBaseType={templateType} />
        case RADIO_BUTTON.value:
            return <RenderRadioQues question={question} isDesktopPreview={isDesktopPreview} templateBaseType={templateType} />
        case USER_INPUT.value:
            return <RenderUserInputQues question={question} isDesktopPreview={isDesktopPreview} templateBaseType={templateType} />;
...
*/
const renderQuestionTitle = (question, index, isQuestionTagsVisible, isDesktopPreview) => {
    const isQuesMandatory = question?.isMandatory;
    return (
        <Box className={clsx({ "template-preview-modal__question-title": true, "template-preview-modal__question-title--mobile": !isDesktopPreview })}>
            <Box>
                {isQuesMandatory && <Typography component="span" className="question-item__required">*</Typography>}
...
*/
export const renderChecklistComponent = (question, index, isDesktopPreview, templateType) => {
    return (
        <Box className={clsx({ "template-preview-modal__question-wrapper": true, "template-preview-modal__question-wrapper--mobile": !isDesktopPreview })}>
            {
                question?.subQuestions?.length > 0 ? 
                <RenderSection question={question} parentIndex={index} isDesktopPreview={isDesktopPreview} templateBaseType={templateType} renderChecklistComponent={renderChecklistComponent} /> :
                <>
                    {question?.questionType !== QUESTION_TYPES.TITLE.value && renderQuestionTitle(question, index, question.questionType === QUESTION_TYPES.LABEL.value, isDesktopPreview)}
...
*/
export const renderGridContainer = (quesData, isDesktopPreview, templateType) => {
    return (
        <TableContainer>
            <Table className={clsx({ "template-preview-modal__grid-table": true, "template-preview-modal__grid-table--mobile": !isDesktopPreview })} data-responsive="true">
