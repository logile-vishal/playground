...
/**
 * @class VariableBlot
 * @description Custom Quill blot for manually inserted variables
 * Displays as {{value}} format with styling applied
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const Inline = Quill.import("blots/inline") as any;
export class VariableBlot extends Inline {
...
/**
 * @class WildcardLabel
 * @description Custom Quill blot for auto-detected wildcard variables
 * Stores the wildcard pattern (%abc%) but displays the label (Question 2 Content)
 * Cannot be edited or have text typed inside it
 * Uses Atomic blot type to prevent Quill from normalizing/modifying the blot
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const Embed = Quill.import("blots/embed") as any;
export class WildcardLabel extends Embed {
  static create(value: string): HTMLElement {
    const node = document.createElement("span");
    node.setAttribute("class", "ql-wildcard");
    node.setAttribute("data-wildcard", value);
    node.setAttribute("contenteditable", "true");
    node.setAttribute("spellcheck", "false");

    // Get the label from the map
    const label = WILD_CARD_MAP.get(value) || value;
    node.innerHTML = CurlyBracketIcon + `<span class='label'>${label}</span>`;

    // Add pointer-events: none to prevent any interaction
    (node.style as unknown as Record<string, string>)["pointerEvents"] = "none";
    (node.style as unknown as Record<string, string>)["userSelect"] = "none";
    (node.style as unknown as Record<string, string>)["cursor"] = "default";

    return node;
  }

  static value(node: HTMLElement): string {
    return node.getAttribute("data-wildcard") || "";
  }

  /**
   * @method isEditable
   * @description Makes wildcard blots immutable - prevents editing inside them
   * @return {boolean} Always false to prevent editing
   */
  isEditable(): boolean {
    return false;
  }
...
// Configure blot names and tags
WildcardLabel.blotName = "wildcard";
WildcardLabel.tagName = "span";
WildcardLabel.className = "ql-wildcard";

VariableBlot.blotName = "variable1";
VariableBlot.tagName = "span";
VariableBlot.className = "ql-variable";

// Register blots with Quill
Quill.register(VariableBlot);
Quill.register(WildcardLabel);
